...
This textbook on regression models in `Stata` is also required. You can search for it online, in the bookstore, and view the publishers website via this ISBN: [978-1597181075](https://www.stata.com/bookstore/interpreting-visualizing-regression-models/). You may consider an earlier edition, but do not expect me to translate any differences between the versions. 

```{r, echo = FALSE, results="asis"} 
bib["Mitchell:2012"]
``` 

This textbook is recommended. You can search for it online, in the bookstore, and view the publishers website via this ISBN: [978-1316642672](https://doi.org/10.1017/9781108131704). You may consider an earlier edition, but do not expect me to translate any differences between the versions. 

```{r, echo = FALSE, results="asis"} 
bib["Kellstedt:2018"]
``` 

This companion book on using `Stata` is also recommended. You can search for it online, in the bookstore, and view the publishers website via this ISBN: [978-1108683791](https://doi.org/10.1017/9781108683791).
...
# kable(dt2, "latex", booktabs = T) %>% kable_styling(latex_options = "striped")
```
...
Important: class readings are subject to change, contingent on mitigating circumstances and the progress we make as a class.  Students are encouraged to check the announcements section and any emails from me for updates. 

## `r advdate(mon, 1)`: Introduction | Review of Linear Regression (in scalar notation) and Matrix Algebra 
...
- Kellstedt & Whitten Chapters 9--11

### Assignments

- Week 1 assignment due Sunday @11:59PM
    
## `r advdate(mon, 2)`: The Linear Model in Matrix Algebra: A Look Under the Hood
...
- Week 2 assignment due Sunday @11:59PM

##  `r advdate(mon, 3)`: The Linear Model in Practice I: Assumptions, Violations, and Generalizations

### Read:

- Wooldridge Chapters 1--4 (review as needed)

### Assignments

- Midterm Exam must be completed by Sunday @11:59PM

##  `r advdate(mon, 4)`: The Linear Model in Practice II: Interactions, Marginal Effects, and Simulating Quantities of Interest
...
- Week 4 assignment due Sunday @11:59PM



##  `r advdate(mon, 5)`: Time Series: Dynamic Political Analysis

### Read:

- Wooldridge Chapters 1--4 (review as needed)

- Kellstedt & Whitten Chapter 12 (second half)

### Assignments

- Week 5 assignment due Sunday @11:59PM

##  `r advdate(mon, 6)`: Pooled Time Series: Merging Time and Space
...
- Week 6 assignment due Sunday @11:59PM

##  `r advdate(mon, 7)`: Mid-Term Exam
...
##  `r advdate(mon, 8)`: Limited Dependent Variables I: Binary Response Models

- Wooldridge Chapter 15

##  `r advdate(mon, 9)`: Limited Dependent Variables II: Ordered, Multinomial, and Count Models

- Wooldridge Chapter 16, 18

##  `r advdate(mon, 10)`: Spatial Models I: Introduction to Interdependence 
...
##  `r advdate(mon, 11)`: Spatial Models II: Advanced Spatial Econometric Strategies
...
##  `r advdate(mon, 12)`: Spatial Models III

##  `r advdate(mon, 13)`: Causal Inference in Political Science: Fad or Forever?

##  `r advdate(mon, 14)`: Fall Break
### Read:
...
##  `r advdate(mon, 15)`: The Research Process in Practice I: The Sausage Factory

### Read:
```{r, echo = FALSE, results="asis"}
bib["Gelman:2006"]
```
[DOWNLOAD](https://www.dropbox.com/s/ilip0717b4wl900/The%20Difference%20Between%20Significant%20and%20Not%20Significant%20is%20not%20Itself%20Statistically%20Significant.pdf?dl=1)

```{r, echo = FALSE, results="asis"}
bib["mcshane:2019"]
```
[DOWNLOAD](https://www.dropbox.com/s/l9yqgoysdu2hphb/Abandon%20Statistical%20Significance.pdf?dl=1)

```{r, echo = FALSE, results="asis"}
bib["Troeger:2019"]
```

##  `r advdate(mon, 16)`: The Research Process in Practice II: Tools of the Trade

- Literate Programming with Markdown and \LaTeX  
